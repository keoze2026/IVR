...
pytest tests/test_tenancy.py tests/test_api_key_writes.py tests/test_webhooks.py -q
```

`test_api_key_writes.py` drives the real URL conf with a real API key. The
faults it covers — a relation resolved across tenants, an `APIKeyUser` assigned
to a `User` column, a repeat opt-out hitting a conditional unique constraint —
all live in the view and serialiser layer, which the pure-logic tier never
touches.

`test_webhooks.py` covers the carrier surface without a carrier. It generates
Twilio HMAC-SHA1 and Telnyx Ed25519 signatures from the providers' published
algorithms rather than by calling the same helper the application verifies
with, so an error in the implementation cannot cancel itself out, and asserts
that tampered, unsigned, wrongly keyed, stale and URL-shifted requests are all
refused. It also pins the two behaviours whose failure is silent: a press-9
opt-out is committed *and* invalidates the pre-dial cache before the response
is returned, and a repeated status callback is not processed twice.

Four of the eight Tier 5 table rows below are now covered here. The rest still
need a carrier.

`makemigrations --check` is the one to keep in CI — it fails the build when a
model change lands without the matching migration, which is what stops the
models and the schema drifting apart.
...
Model migrations **are** checked in, as normal for a Django project. They were
not, originally: the environment this was authored in had no Django installed,
and a hand-written migration that silently disagrees with `models.py` on an
index name or a constraint condition is worse than no migration at all. That
constraint no longer applies — the migrations in the tree were generated by
`makemigrations` against the real models and verified by `makemigrations
--check` in CI, so the models and the schema cannot drift apart silently.

Generating them per environment was never safe for production anyway: two
environments running `makemigrations` at different times can produce different
schemas from the same models, and a data migration has nowhere to live.

Everything the ORM *cannot* express — the
